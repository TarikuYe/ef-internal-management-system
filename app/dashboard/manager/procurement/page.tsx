...
export default async function ProcurementManagerPage() {
  ...
  // Security guard: must be admin, dgm, or procurement manager
  if (
    employee.role !== 'admin' &&
    employee.role !== 'dgm' &&
    !(employee.role === 'manager' && employee.department_id === 'procurement')
  ) {
    ...
    <div className="flex min-h-screen flex-col">
        ...
        <div className="flex flex-col gap-6">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Review and coordinate procurement lifecycles, RFQs, vendor comparisons, and purchase approvals.
            </p>
          </div>
          ...
          <Card className="border-dashed border-2 border-border">
            <CardHeader className="flex flex-col items-center justify-center text-center p-12">
              ...
              <CardDescription className="max-w-md mt-2 text-sm">
                The Procurement manager RFQ comparisons, vendor scores, and purchase approval pipelines are currently undergoing system discovery and database mapping.
              </CardDescription>
      ...
      <footer className="border-t border-border bg-secondary/40 mt-auto">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-4 py-6 text-sm text-muted-foreground sm:flex-row sm:px-6">
          ...
          <p>EF Management Portal — Procurement Manager Dashboard</p>
        </div>
      </footer>
    </div>
  )
